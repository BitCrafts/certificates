...
public class LeafCertificateService_Server_Tests : IAsyncLifetime
{
    ...
    {
        _root = TestDataRoot.Create(nameof(LeafCertificateService_Server_Tests));
    ...
    [Fact]
    public async Task IssueServer_Creates_Files_And_Db_Row()
    {
        var settings = new SettingsRepository(_factory);
        var ca = new CaService(_data, settings, NullLogger<CaService>.Instance);
        ...
        var id = await svc.IssueServerAsync("web.test.lan", new[] { "10.0.0.10", "10.0.0.11" });

        // Files
        var dir = Path.Combine(_data.CertsServersDir, "web.test.lan");
        ...
        rec!.Kind.Should().Be("server");
        rec.SanDns.Should().Be("web.test.lan");
        rec.SanIp.Should().Contain("10.0.0.10").And.Contain("10.0.0.11");
        rec.Status.Should().Be("active");
        rec.CertPath.Should().EndWith("cert.crt");
        rec.KeyPath.Should().EndWith("key.pem");
    }
}
